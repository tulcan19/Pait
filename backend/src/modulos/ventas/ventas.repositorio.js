...
async function crearVenta(client, { id_cliente, total, id_usuario }) {
  const sql = `
    INSERT INTO ventas (id_cliente, total, id_usuario, estado)
    VALUES ($1, $2, $3, 'registrada')
    RETURNING *;
  `;
  const { rows } = await client.query(sql, [id_cliente, total, id_usuario]);
  return rows[0];
}
